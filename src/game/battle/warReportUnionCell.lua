module("WarReportUnionCell", package.seeall)
--同盟战报界面的cell
...
function create(widget, cell_idx, parentPanel)
	...
	--我方名字
	local myname = tolua.cast(panel:getChildByName("Label_24542_0_0_0_0"), "Label")
	--我方同盟名字
	local myUnionname = tolua.cast(panel:getChildByName("Label_31665_0"), "Label")
	--我方是否有同盟
	local myUnionImage = tolua.cast(panel:getChildByName("ImageView_143691_0"), "ImageView")
    ...
    --对方是否是守军
    local shoujun_str = tolua.cast(panel:getChildByName("Label_413699"),"Label")
    ...
    --我方部队底盘颜色
    local ownArmyColor = tolua.cast(ownPanel:getChildByName("ImageView_891802_2"),"ImageView")
    ...
    if reportData.returnAttackOrDefend( battleReport) then
        ...
        if battleReport.attack_name == userData.getUserName() then
            ownArmyColor:loadTexture(ResDefineUtil.army_icon_res[3][2],UI_TEX_TYPE_PLIST)
        else
            ownArmyColor:loadTexture(ResDefineUtil.army_icon_res[2][2],UI_TEX_TYPE_PLIST)
        end
        ...
        if battleReport.defend_name == userData.getUserName() then
            ownArmyColor:loadTexture(ResDefineUtil.army_icon_res[3][2],UI_TEX_TYPE_PLIST)
        else
            ownArmyColor:loadTexture(ResDefineUtil.army_icon_res[2][2],UI_TEX_TYPE_PLIST)
        end
