...
local function do_remove_self()
	if card_overview_layer then
		...
		uiManager.remove_self_panel(uiIndexDefine.CARD_PACKET_UI)
	end
end

local function remove_self()
    if m_backPanel then
    	uiManager.hideConfigEffect(uiIndexDefine.CARD_PACKET_UI, card_overview_layer, do_remove_self, 999, {m_backPanel:getMainWidget()})
    end
end
...
local function update_sort_panel_state(new_state)
	-- card_sort_widget:setVisible(new_state)
	-- card_sort_widget:setTouchEnabled(new_state)
	...
	cardPacketManager.reload_data()

	-- local temp_btn = nil
	-- for i=1,5 do
	-- 	temp_btn = tolua.cast(card_sort_widget:getChildByName("btn_" .. i), "Button")
	-- 	temp_btn:setTouchEnabled(new_state)
	-- end
...
local function show_cell_content(cell_widget, idx)
	...
	local function deal_with_card_click(sender)
		if is_response_touch_end() then
			...
			local temp_hero_id = m_hero_id_list[start_index + select_index - 1]
			if temp_hero_uid ~= 0 then
				--update_tb_state(false)
				require("game/cardDisplay/basicCardViewer")
				if m_hero_basic_lv == 1 then
					basicCardViewer.create(nil, temp_hero_id)
				else
					basicCardViewer.create(nil, temp_hero_id, nil, m_hero_basic_lv)
				end
			end
		end
	end
	...
	for j=start_index,end_index do
		...
		local hero_widget = tolua.cast(hero_panel:getChildByName("hero_icon"), "Layout")
		local hero_id = m_hero_id_list[j]
		cardFrameInterface.set_middle_card_info(hero_widget, 0, hero_id)
		if m_hero_basic_lv ~= 1 then
			cardFrameInterface.set_lv_images(m_hero_basic_lv, hero_widget)
		end
...
local function create()
	...
	local all_widget = m_backPanel:create(temp_widget, remove_self, panelPropInfo[uiIndexDefine.CARD_PACKET_UI][2], false, true)
	...
	local card_list_panel = tolua.cast(temp_widget:getChildByName("content_panel"), "Layout")
	m_card_tb_view = CCTableView:create(CCSizeMake(card_list_panel:getSize().width, card_list_panel:getSize().height))
    ...
    uiManager.add_panel_to_layer(card_overview_layer, uiIndexDefine.CARD_PACKET_UI)
    uiManager.showConfigEffect(uiIndexDefine.CARD_PACKET_UI, card_overview_layer, nil, 999, {all_widget})
...
local function reload_data()
	cardSortManager.sort_fun_for_cid(m_hero_id_list, sort_type)

	m_card_tb_view:reloadData()

	local num_txt = tolua.cast(temp_widget:getChildByName("num_label"), "Label")
	num_txt:setText(#m_hero_id_list)
end

local function is_table_element(temp_value)
	for k,v in pairs(m_hero_id_list) do
...
local function merge_extract_package_to_parent(temp_refresh_id)
	for k,v in pairs(cardExtractPacket[temp_refresh_id]) do
		if not is_table_element(v) then
			table.insert(m_hero_id_list, v)
		end
	end
end

local function organize_ex_package_card_list(temp_refresh_id)
	local temp_refresh_info = Tb_cfg_card_extract[temp_refresh_id]
	m_hero_basic_lv = cardCallData.get_extract_card_level(temp_refresh_id)

	m_hero_id_list = cardExtractPacket[temp_refresh_id]
	if temp_refresh_info.parent_extract_id == 0 then
		local current_time = userData.getServerTime()
		for k,v in pairs(allTableData[dbTableDesList.sys_card_extract_combine.name]) do
			if Tb_cfg_card_extract[v.extract_id].parent_extract_id == temp_refresh_id then
				if v.combine_end_time == 0 or v.combine_end_time < current_time then
					merge_extract_package_to_parent(v.extract_id)
				end
			end
		end
	end
...
local function enter_packet_overview(temp_refresh_id)
	if card_overview_layer then
		return
	end

	organize_ex_package_card_list(temp_refresh_id)
	create()
	reload_data()
end

cardPacketManager = {
						enter_packet_overview = enter_packet_overview,
