...
local function do_remove_self()
	if m_pMainLayer then 
        ...
        m_pMainLayer = nil
        uiManager.remove_self_panel(uiIndexDefine.CARD_VIEWER_USER)
...
function remove_self(closeEffect)
	...
	uiManager.hideConfigEffect(uiIndexDefine.CARD_VIEWER_USER,m_pMainLayer,do_remove_self)
end
...
function create(uidList,uid,callback)
	...
	local afterShowFun = nil
    m_fHeroCardUpdator,m_fHeroCardDeleted,m_fHeroCardAdded,m_fCardViewDispose,m_fGetMainWidget,afterShowFun = cardViewer.setCardViewer2Layer( cardViewer.VIEW_TYPE_USER,uidList,uid,m_pMainLayer,remove_self)
    uiManager.add_panel_to_layer(m_pMainLayer, uiIndexDefine.CARD_VIEWER_USER)
	...
	uiManager.showConfigEffect(uiIndexDefine.CARD_VIEWER_USER,m_pMainLayer,function()
		m_pMainLayer:getParent():setOpacity(230)
		if afterShowFun then 
			afterShowFun()
		end
	end)

	-- if  m_iNonForcedGuideId == com_guide_id_list.CONST_GUIDE_2028  then 
 -- 		comGuideManager.set_show_guide(com_guide_id_list.CONST_GUIDE_2028)
 -- 	end
 	if m_iNonForcedGuideId then 
 		comGuideManager.set_show_guide(m_iNonForcedGuideId)
 		m_iNonForcedGuideId = nil
 	end
