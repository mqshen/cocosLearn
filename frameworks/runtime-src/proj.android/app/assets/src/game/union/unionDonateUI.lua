--同盟的捐赠界面
module("UnionDonateUI", package.seeall)
...
local function do_remove_self( )
	if m_pMainLayer then
		...
		UIUpdateManager.remove_prop_update(dbTableDesList.user_union_attr.name, dataChangeType.update, UnionDonateUI.init_all_content_show)
		uiManager.remove_self_panel(uiIndexDefine.UNION_DONATE_UI)
		UnionMainUI.setTouchEnable(true)
	end
end

function remove_self()
	if not m_pMainLayer then return end
	uiManager.hideConfigEffect(uiIndexDefine.UNION_DONATE_UI,m_pMainLayer,do_remove_self)
	-- uiManager.hideScaleEffect(m_pMainLayer,999,do_remove_self,nil)
end
...
local function init_donate_ui(temp_widget)
	...
	-- name_txt_1:setText(languagePack["tongmengdengji"])
	-- local name_txt_2 = tolua.cast(content_panel:getChildByName("sign_3"), "Label")
	-- name_txt_2:setText(languagePack["juanzengjingyan"])

	set_res_show_content(true)
	-- set_btn_show_content(languagePack["querenjuanzeng"])
end
...
local function init_param_info()
	...
	if m_bDonate then
		...
		m_rebel_need_nums = UnionData.getRebelData().total - UnionData.getRebelData().hasDonate
	end
end
...
local function organize_first_content(isOpreating)
	...
	local loading_bar_expected = uiUtil.getConvertChildByName(content_panel,"loading_bar_expected")
	loading_bar_expected:setVisible(false)
	if m_bDonate then
		local unionInfo = UnionData.getUnionInfo()
		local new_level, new_exp, new_all_exp = compute_exp_info(unionInfo.level, unionInfo.exp, math.floor(sum_num/100))
		local lv_txt = tolua.cast(content_panel:getChildByName("lv_label"), "Label")
		lv_txt:setText(languagePack["lv"] .. new_level)
		-- before_txt:setText(unionInfo.exp)
		after_txt:setStringValue(new_exp .. "/" .. new_all_exp)
		if isOpreating then 
			loading_bar_expected:setVisible(true)
			loading_bar_expected:setPercent(math.floor(100 * new_exp / new_all_exp))
		else
			loading_bar:setPercent(math.floor(100 * new_exp / new_all_exp))
		end
	end
...
local function init_ui()
	...
	-- if m_bDonate then
		-- title_txt:setText(languagePack["tongmengjuanzeng"])
		init_donate_ui(temp_widget)
	-- end
...
function create(isDonate)
	...
	local widget = GUIReader:shareReader():widgetFromJsonFile("test/The_alliance_xinjuanxian_0.json")
	widget:setTag(999)
	...
	uiManager.add_panel_to_layer(m_pMainLayer, uiIndexDefine.UNION_DONATE_UI)

	init_ui()
	UIUpdateManager.add_prop_update(dbTableDesList.user_union_attr.name, dataChangeType.update, UnionDonateUI.init_all_content_show)
	uiManager.showConfigEffect(uiIndexDefine.UNION_DONATE_UI,m_pMainLayer)
end
